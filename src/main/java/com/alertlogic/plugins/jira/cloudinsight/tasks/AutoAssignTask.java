...
public class AutoAssignTask implements PluginJob {
        ...
        if (rulesConfigured != null) 
        {
            ...
            for ( int i = 0; i < rulesConfigured.length(); i++ )
            {
                ...
                if (rule != null) {
                    ...
                    String ruleActingAccountId = rule.getString("aaid");

                    TaskLogger taskLogger = new TaskLogger();
                    taskLogger.log(monitor.getI18nResolver().getText("ci.job.autoassign.msg.log.header") + ruleName+ " Account ("+ruleActingAccountId+")");
    ...
    private void manageRuleException(JSONObject rule,RuleConfig ruleConfig,Exception e,TaskLogger taskLogger,int statusBeforeRun) 
    {
        ...
        String actingAccountId = rule.has("aaid") ? rule.getString("aaid") : "No acting account id available.";
        ...
        if (statusBeforeRun == TaskRuleExecutionState.ERROR) 
        {
            ...
            taskLogger.log(monitor.getI18nResolver().getText("ci.job.autoassign.msg.log.blocked") + ruleName +"("+actingAccountId+"):"+e.getMessage());
            taskLogger.log(monitor.getI18nResolver().getText("ci.job.autoassign.msg.log.blocked") + ruleName +"("+actingAccountId+"):"+monitor.getI18nResolver().getText("ci.job.autoassign.msg.log.blocked.msg"));
            ...
            //Prepare the text for logging
            taskLogger.log(monitor.getI18nResolver().getText("ci.job.autoassign.msg.log.stacktrace") + exceptionAsString);
            taskLogger.log(monitor.getI18nResolver().getText("ci.job.autoassign.msg.log.error")+ ruleName +"("+actingAccountId+"):"+e.getMessage());
